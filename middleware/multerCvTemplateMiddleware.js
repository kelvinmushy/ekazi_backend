...
const storage = multer.diskStorage({
    destination: (req, file, cb) => {
        cb(null, 'uploads/cvtemplates'); // The folder where you want to save the uploaded files
    },
...
const template = multer({
    storage: storage,
    limits: { fileSize: 5 * 1024 * 1024 }, // 5MB max file size
    fileFilter: (req, file, cb) => {
        const filetypes = /jpg|jpeg|png|gif|bmp|svg/;// Accepted image formats
